...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.V = np.mean((self.Z-self.M)**2, axis=0)  #broadcasting is doing the job rn # TODO

        if eval == False:
            # training mode
            self.NZ = (self.Z-self.M)/np.sqrt(self.V + self.eps) #wooh broadcasting  # TODO
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -1/2 * np.sum(dLdNZ * (self.Z-self.M) * (self.V-self.eps)**(-3/2), axis=0, keepdims=True)
        temp = (1/2*(self.Z-self.M)) * ((self.V + self.eps)**(-3/2)) * -2/self.N * np.sum(self.Z-self.M, axis=0, keepdims=True)
        dNZdM = (-(self.V + self.eps)**(-1/2)) - temp
        dLdM = np.sum(dLdNZ * dNZdM, axis=0, keepdims=True)
        dLdZ = (dLdNZ * ((self.V + self.eps)**(-1/2)).T)+ (2/self.N * (self.Z-self.M) * dLdV + 1/self.N * dLdM)  # TODO
        # was a pain to type/shape check: use sum for summation after component wise mult
        return dLdZ
